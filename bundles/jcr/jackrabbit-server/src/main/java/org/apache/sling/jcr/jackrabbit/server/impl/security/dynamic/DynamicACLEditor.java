 ...
 * <code>ACLEditor</code>...
 */
public class DynamicACLEditor extends DynamicSecurityItemModifier implements AccessControlEditor, AccessControlConstants {

    /**
     * the default logger
     */
    private static final Logger log = LoggerFactory.getLogger(DynamicACLEditor.class);
    /**
    ...
    DynamicACLEditor(Session editingSession, AccessControlUtils utils) {
    ...
    AccessControlList getACL(NodeImpl aclNode) throws RepositoryException {
        return new DynamicACLTemplate(aclNode, privilegeRegistry);
    }
    ...
    public AccessControlPolicy[] editAccessControlPolicies(String nodePath) throws AccessControlException, PathNotFoundException, RepositoryException {
        ...
        if (aclNode == null) {
            // create an empty acl
            acl = new DynamicACLTemplate(nodePath, session.getPrincipalManager(), privilegeRegistry);
        } else {
            acl = getACL(aclNode);
        }
    ...
    public void setPolicy(String nodePath, AccessControlPolicy policy) throws RepositoryException {
        ...
        AccessControlEntry[] entries = ((DynamicACLTemplate) policy).getAccessControlEntries();
    ...
    private static void checkValidPolicy(String nodePath, AccessControlPolicy policy) throws AccessControlException {
        if (policy == null || !(policy instanceof DynamicACLTemplate)) {
            throw new AccessControlException("Attempt to set/remove invalid policy " + policy);
        }
        DynamicACLTemplate acl = (DynamicACLTemplate) policy;
    ...
    private NodeImpl getAclNode(String nodePath) throws PathNotFoundException, RepositoryException {
        ...
        if (DynamicACLProvider.isAccessControlled(protectedNode)) {
