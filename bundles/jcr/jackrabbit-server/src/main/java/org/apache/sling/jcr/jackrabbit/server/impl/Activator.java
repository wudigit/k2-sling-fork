...
import org.apache.sling.jcr.jackrabbit.server.impl.security.dynamic.DynamicPrincipalManagerFactory;
import org.apache.sling.jcr.jackrabbit.server.impl.security.dynamic.DynamicPrincipalManagerFactoryImpl;
...
public class Activator implements BundleActivator, ServiceListener {
  ...
  /**
   * The name of the configuration property naming the Sling Context for which a factory
  ...
  public void stop(BundleContext arg0) {
    ...
    /*
     * when stopping Derby (which is used by Jackrabbit by default) a derby.antiGC thread
     * keeps running which prevents this bundle from being garbage collected ... we try to
     * really stop derby here and ignore the exception since according to
     * http://db.apache.org/derby/docs/10.4/devguide/tdevdvlp20349.html this exception
  ...
  /**
   * Returns the registered {@link LoginModulePlugin} services. If there are no
   * {@link LoginModulePlugin} services registered, this method returns an empty array.
   * <code>null</code> is never returned from this method.
   */
  public static LoginModulePlugin[] getLoginModules() {
    ...
    if (loginModuleTracker == null) {
      loginModuleTracker = new ServiceTracker(getBundleContext(), LoginModulePlugin.class
          .getName(), null);
  ...
  private void verifyConfiguration(ServiceReference ref) {
    ...
    if (ca == null) {
      log
          .error("verifyConfiguration: Failed to get Configuration Admin Service from Service Reference");
      return;
    }
    ...
    try {
      ...
      Configuration[] cfgs = ca.listConfigurations("("
          + ConfigurationAdmin.SERVICE_FACTORYPID + "=" + SERVER_REPOSITORY_FACTORY_PID
          + ")");
      if (cfgs != null && cfgs.length > 0) {
        log.info(
            "verifyConfiguration: {} Configurations available for {}, nothing to do",
            new Object[] {new Integer(cfgs.length), SERVER_REPOSITORY_FACTORY_PID});
        return;
      }

      // No config, create a default one.
      Hashtable<String, String> defaultConfig = new Hashtable<String, String>();
      final String overrideUrl = bundleContext
          .getProperty(RepositoryAccessor.REPOSITORY_URL_OVERRIDE_PROPERTY);
      ...
      log.info("verifyConfiguration: Created configuration {} for {}", config.getPid(),
  ...
  private void initDefaultConfig(Hashtable<String, String> props,
    ...
    props.put(SlingServerRepository.REPOSITORY_REGISTRATION_NAME, this
        .getRepositoryName());
  }
  ...
  private File getHomeDir(BundleContext bundleContext) throws IOException {
    ...
    if (!homeDir.isDirectory()) {
      if (!homeDir.mkdirs()) {
        log.info("verifyConfiguration: Cannot create Jackrabbit home " + homeDir
  ...
  private File getConfigFile(BundleContext bundleContext, File homeDir)
    ...
    String repoConfigFileUrl = bundleContext
        .getProperty("sling.repository.config.file.url");
    ...
    SlingServerRepository.copyFile(bundleContext.getBundle(), "repository.xml",
        configFile);
    return configFile;
  }
